<!--  -->
<style lang='less' scoped>
@import '../../styles.less';
</style>

<template>
  <div>
    <chart :options="options" auto-resize ref="myChart" class="chart"></chart>
  </div>
</template>

<script>
import _ from 'underscore'
import * as dataConfig from './dataConfig'

export default {
  name: 'southEcharts',
  data () {
    return {
    }
  },

  components: {},

  computed: {
    // 获取接口数据
    siteStat () {
      return this.$store.state.home.siteStat
    },
    options () {
      return this.$store.state.home.southOption
    }
  },

  methods: {
    initSouth () {
      // let southOption = dataConfig.INIT_OPTION;
      // this.$store.commit('SOUTH_OPTION', southOption);
      this.getDataApi()
    },
    getDataApi: async function () {
      // 请求接口
      // await this.$store.dispatch('dimensionality');
      if (!_.isEmpty(this.siteStat)) {
        this.buildSeries(this.siteStat)
      }
    },
    buildSeries () {
      let arg = arguments[0]
      // 组装option数据
      let southOption = dataConfig.process(arg)
      // 给页面option赋值
      this.$store.commit('SOUTH_OPTION', southOption)
    }
  },
  watch: {
    siteStat (val) {
      if (val) {
        this.initSouth()
      }
    }
  }
}

</script>
